...
#define NOVATEK_GPIO_BANK_WIDTH			32

#define NOVATEK_GPIO_BANK_STRIDE		0x4
#define NOVATEK_GPIO_BANK_OFS(pin) \
	(((pin) / NOVATEK_GPIO_BANK_WIDTH) * NOVATEK_GPIO_BANK_STRIDE)

#define NOVATEK_GPIO_DATA_BASE			0x00
#define NOVATEK_GPIO_DATA_OFS(pin) \
	(NOVATEK_GPIO_DATA_BASE + NOVATEK_GPIO_BANK_OFS(pin))
#define NOVATEK_GPIO_DATA_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_DATA_LOW			0x0
#define NOVATEK_GPIO_DATA_HIGH			0x1

#define NOVATEK_GPIO_DIR_BASE			0x20
#define NOVATEK_GPIO_DIR_OFS(pin) \
	(NOVATEK_GPIO_DIR_BASE + NOVATEK_GPIO_BANK_OFS(pin))
#define NOVATEK_GPIO_DIR_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_DIR_INPUT			0x0
#define NOVATEK_GPIO_DIR_OUTPUT			0x1

#define NOVATEK_GPIO_SET_BASE			0x40
#define NOVATEK_GPIO_SET_OFS(pin) \
	(NOVATEK_GPIO_SET_BASE + NOVATEK_GPIO_BANK_OFS(pin))
#define NOVATEK_GPIO_SET_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_SET_KEEP			0x0
#define NOVATEK_GPIO_SET_HIGH			0x1

#define NOVATEK_GPIO_CLR_BASE			0x60
#define NOVATEK_GPIO_CLR_OFS(pin) \
	(NOVATEK_GPIO_CLR_BASE + NOVATEK_GPIO_BANK_OFS(pin))
#define NOVATEK_GPIO_CLR_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_CLR_KEEP			0x0
#define NOVATEK_GPIO_CLR_LOW			0x1

#define NOVATEK_GPIO_IRQ_STATUS_OFS		0x80
#define NOVATEK_GPIO_IRQ_STATUS_MASK(bit)	BIT(bit)
#define NOVATEK_GPIO_IRQ_STATUS_KEEP		0x0
#define NOVATEK_GPIO_IRQ_STATUS_CLEAR		0x1
#define NOVATEK_GPIO_IRQ_STATUS_NONE		0x0
#define NOVATEK_GPIO_IRQ_STATUS_PENDING		0x1

#define NOVATEK_GPIO_IRQ_ENABLE_OFS		0x90
#define NOVATEK_GPIO_IRQ_ENABLE_MASK(bit)	BIT(bit)
#define NOVATEK_GPIO_IRQ_DISABLE		0x0
#define NOVATEK_GPIO_IRQ_ENABLE			0x1
#define NOVATEK_GPIO_IRQ_DISABLE_ALL		0x00000000

#define NOVATEK_GPIO_IRQ_TYPE_OFS		0xa0
#define NOVATEK_GPIO_IRQ_TYPE_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_IRQ_TYPE_EDGE		0x0
#define NOVATEK_GPIO_IRQ_TYPE_LEVEL		0x1

#define NOVATEK_GPIO_IRQ_POLARITY_OFS		0xa4
#define NOVATEK_GPIO_IRQ_POLARITY_MASK(bit)	BIT(bit)
#define NOVATEK_GPIO_IRQ_POLARITY_NORMAL	0x0
#define NOVATEK_GPIO_IRQ_POLARITY_INVERTED	0x1

#define NOVATEK_GPIO_IRQ_EDGE_OFS		0xa8
#define NOVATEK_GPIO_IRQ_EDGE_MASK(bit)		BIT(bit)
#define NOVATEK_GPIO_IRQ_EDGE_SINGLE		0x0
#define NOVATEK_GPIO_IRQ_EDGE_BOTH		0x1

#define NOVATEK_GPIO_DGPIO_IRQ_STATUS_OFS	0xc0

#define NOVATEK_GPIO_DGPIO_IRQ_ENABLE_OFS	0xd0

#define NOVATEK_GPIO_DGPIO_IRQ_TYPE_OFS		0xe0

#define NOVATEK_GPIO_DGPIO_IRQ_POLARITY_OFS	0xe4

#define NOVATEK_GPIO_DGPIO_IRQ_EDGE_OFS		0xe8

#define NA51089_GPIO_CGPIO_NUM_PINS		23
#define NA51089_GPIO_PGPIO_NUM_PINS		26
#define NA51089_GPIO_SGPIO_NUM_PINS		9
#define NA51089_GPIO_LGPIO_NUM_PINS		10
#define NA51089_GPIO_DGPIO_NUM_PINS		8
#define NA51089_GPIO_HGPIO_NUM_PINS		12
#define NA51089_GPIO_AGPIO_NUM_PINS		3
#define NA51089_GPIO_DSIGPIO_NUM_PINS		11
...
struct novatek_gpio_irq_bank {
	...
	u32 valid_mask;
	uint resource;
	uint pins[NOVATEK_GPIO_BANK_WIDTH];
};
	...
	struct novatek_gpio *gpio, uint offset, uint *bit)
{
	...
	for (i = 0; i < soc->num_banks; i++) {
		...
		*bit = offset - bank->pin_base;
		if (*bit < NOVATEK_GPIO_BANK_WIDTH &&
	...
	struct novatek_gpio *gpio, uint offset, uint *bit)
{
	...
	for (i = 0; i < soc->num_irq_banks; i++) {
		...
		for (*bit = 0; *bit < NOVATEK_GPIO_BANK_WIDTH; (*bit)++) {
					 ...
					 unsigned int offset, int value)
{
	...
	if (!bank)
		return -EINVAL;
	raw_spin_lock_irqsave(&gpio->lock, flags);
	writel((u32)(value ? NOVATEK_GPIO_SET_HIGH :
			   NOVATEK_GPIO_CLR_LOW) << bit,
	       gpio->bases[bank->resource] + (value ? bank->set : bank->clr));

	reg = readl(gpio->bases[bank->resource] + bank->dir);
	reg |= (u32)NOVATEK_GPIO_DIR_OUTPUT << bit;
	writel(reg, gpio->bases[bank->resource] + bank->dir);
	raw_spin_unlock_irqrestore(&gpio->lock, flags);
...
static void novatek_gpio_irq_ack(struct irq_data *d)
{
	...
	if (!bank)
		return;
	raw_spin_lock_irqsave(&gpio->lock, flags);
	writel((u32)NOVATEK_GPIO_IRQ_STATUS_CLEAR << bit,
...
static void novatek_gpio_irq_mask(struct irq_data *d)
{
	...
	reg &= ~NOVATEK_GPIO_IRQ_ENABLE_MASK(bit);
...
static void novatek_gpio_irq_unmask(struct irq_data *d)
{
	...
	raw_spin_lock_irqsave(&gpio->lock, flags);
	reg = readl(gpio->bases[bank->resource] + bank->enable);
	reg |= (u32)NOVATEK_GPIO_IRQ_ENABLE << bit;
	writel(reg, gpio->bases[bank->resource] + bank->enable);
	raw_spin_unlock_irqrestore(&gpio->lock, flags);
}
...
static int novatek_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	reg = level ? reg | ((u32)NOVATEK_GPIO_IRQ_TYPE_LEVEL << bit) :
		      reg & ~NOVATEK_GPIO_IRQ_TYPE_MASK(bit);
	writel(reg, gpio->bases[bank->resource] + bank->type);

	reg = readl(gpio->bases[bank->resource] + bank->polarity);
	reg = inverted ?
		reg | ((u32)NOVATEK_GPIO_IRQ_POLARITY_INVERTED << bit) :
		reg & ~NOVATEK_GPIO_IRQ_POLARITY_MASK(bit);
	writel(reg, gpio->bases[bank->resource] + bank->polarity);

	reg = readl(gpio->bases[bank->resource] + bank->edge);
	reg = both_edges ? reg | ((u32)NOVATEK_GPIO_IRQ_EDGE_BOTH << bit) :
			   reg & ~NOVATEK_GPIO_IRQ_EDGE_MASK(bit);
...
static int novatek_gpio_irq_init_hw(struct gpio_chip *gc)
{
	...
	for (i = 0; i < soc->num_irq_banks; i++) {
		...
		writel(NOVATEK_GPIO_IRQ_DISABLE_ALL,
...
static const struct novatek_gpio_bank na51089_gpio_banks[] = {
	{
		...
		.valid_mask = GENMASK(NA51089_GPIO_CGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_CGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_CGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_CGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_CGPIO(0)),
	},
	{
		.pin_base = NA51089_PGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_PGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_PGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_PGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_PGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_PGPIO(0)),
	},
	{
		.pin_base = NA51089_SGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_SGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_SGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_SGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_SGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_SGPIO(0)),
	},
	{
		.pin_base = NA51089_LGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_LGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_LGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_LGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_LGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_LGPIO(0)),
	},
	{
		.pin_base = NA51089_DGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_DGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_DGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_DGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_DGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_DGPIO(0)),
	},
	{
		.pin_base = NA51089_HGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_HGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_HGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_HGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_HGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_HGPIO(0)),
	},
	{
		.pin_base = NA51089_AGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_AGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_AGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_AGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_AGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_AGPIO(0)),
	},
	{
		.pin_base = NA51089_DSIGPIO(0),
		.valid_mask = GENMASK(NA51089_GPIO_DSIGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.data = NOVATEK_GPIO_DATA_OFS(NA51089_DSIGPIO(0)),
		.dir = NOVATEK_GPIO_DIR_OFS(NA51089_DSIGPIO(0)),
		.set = NOVATEK_GPIO_SET_OFS(NA51089_DSIGPIO(0)),
		.clr = NOVATEK_GPIO_CLR_OFS(NA51089_DSIGPIO(0)),
	},
};

static const struct novatek_gpio_irq_bank na51089_gpio_irq_banks[] = {
	{
		.status = NOVATEK_GPIO_IRQ_STATUS_OFS,
		.enable = NOVATEK_GPIO_IRQ_ENABLE_OFS,
		.type = NOVATEK_GPIO_IRQ_TYPE_OFS,
		.polarity = NOVATEK_GPIO_IRQ_POLARITY_OFS,
		.edge = NOVATEK_GPIO_IRQ_EDGE_OFS,
		.valid_mask = GENMASK(NOVATEK_GPIO_BANK_WIDTH - 1, 0),
		.resource = 0,
		.pins = {
	...
	{
		.status = NOVATEK_GPIO_DGPIO_IRQ_STATUS_OFS,
		.enable = NOVATEK_GPIO_DGPIO_IRQ_ENABLE_OFS,
		.type = NOVATEK_GPIO_DGPIO_IRQ_TYPE_OFS,
		.polarity = NOVATEK_GPIO_DGPIO_IRQ_POLARITY_OFS,
		.edge = NOVATEK_GPIO_DGPIO_IRQ_EDGE_OFS,
		.valid_mask = GENMASK(NA51089_GPIO_DGPIO_NUM_PINS - 1, 0),
		.resource = 0,
		.pins = {
			[0] = NA51089_DGPIO(0),
...
static const struct novatek_gpio_soc_data na51089_gpio_soc = {
	.ngpios = NA51089_DSIGPIO(NA51089_GPIO_DSIGPIO_NUM_PINS - 1) + 1,
	.num_resources = 1,
