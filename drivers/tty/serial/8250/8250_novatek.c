...
#define NOVATEK_UART_REGSHIFT		2

#define NOVATEK_UART_PSR		(0x08 / sizeof(u32))
#define NOVATEK_UART_PSR_DIV_MASK	GENMASK(4, 0)
#define NOVATEK_UART_PSR_DIV1		0x01

#define NOVATEK_UART_MCR_AFC_MASK	BIT(6)
#define NOVATEK_UART_MCR_AFC_DISABLE	0x0
#define NOVATEK_UART_MCR_AFC_ENABLE	0x1

#define NOVATEK_UART_R485R_OFFSET	0x20
#define NOVATEK_UART_R485R \
	(NOVATEK_UART_R485R_OFFSET / sizeof(u32))
#define NOVATEK_UART_R485R_EN_MASK	BIT(0)
#define NOVATEK_UART_R485R_DISABLE	0x0
#define NOVATEK_UART_R485R_ENABLE	0x1
#define NOVATEK_UART_R485R_SETUP_MASK	GENMASK(15, 4)
#define NOVATEK_UART_R485R_HOLD_MASK	GENMASK(31, 20)
				    ...
				    unsigned int quot, unsigned int quot_frac)
{
	...
	serial_out(up, NOVATEK_UART_PSR,
		   FIELD_PREP(NOVATEK_UART_PSR_DIV_MASK,
			      NOVATEK_UART_PSR_DIV1));
...
static void novatek8250_set_hw_flow(struct uart_port *port, bool enable)
{
	...
	if (enable) {
		up->mcr |= FIELD_PREP(NOVATEK_UART_MCR_AFC_MASK,
				     NOVATEK_UART_MCR_AFC_ENABLE);
		port->status |= UPSTAT_AUTOCTS | UPSTAT_AUTORTS;
	} else {
		up->mcr &= ~NOVATEK_UART_MCR_AFC_MASK;
		port->status &= ~(UPSTAT_AUTOCTS | UPSTAT_AUTORTS);
	}
}
...
static u32 novatek8250_delay_to_bits(u32 delay_ms, uint baud)
{
	...
	return min_t(u64, bits, FIELD_MAX(NOVATEK_UART_R485R_SETUP_MASK));
}
				    ...
				    uint baud)
{
	...
	value = FIELD_PREP(NOVATEK_UART_R485R_EN_MASK,
			   NOVATEK_UART_R485R_DISABLE);
	if (rs485->flags & SER_RS485_ENABLED) {
		...
		value = FIELD_PREP(NOVATEK_UART_R485R_EN_MASK,
				   NOVATEK_UART_R485R_ENABLE) |
			FIELD_PREP(NOVATEK_UART_R485R_SETUP_MASK, setup) |
			FIELD_PREP(NOVATEK_UART_R485R_HOLD_MASK, hold);
	...
	serial_out(up, NOVATEK_UART_R485R, value);
}
	...
	struct earlycon_device *device, const char *options)
{
	...
	device->port.regshift = NOVATEK_UART_REGSHIFT;
...
static int novatek8250_probe(struct platform_device *pdev)
{
	...
	if (has_rtscts || !rs485_at_boot)
		writel(FIELD_PREP(NOVATEK_UART_R485R_EN_MASK,
				  NOVATEK_UART_R485R_DISABLE),
		       up.port.membase + NOVATEK_UART_R485R_OFFSET);

	device_property_read_u32(dev, "current-speed", &data->baud);

	up.port.type = PORT_NOVATEK;
	up.port.iotype = UPIO_MEM32;
	up.port.regshift = NOVATEK_UART_REGSHIFT;
...
static int novatek8250_suspend(struct device *dev)
{
	...
	if (up->port.suspended)
		clk_disable_unprepare(data->clk);

	return 0;
}

static int novatek8250_resume(struct device *dev)
{
	struct novatek8250_data *data = dev_get_drvdata(dev);
	struct uart_8250_port *up = serial8250_get_port(data->line);
	int ret;
