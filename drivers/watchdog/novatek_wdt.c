...
#define WDT_CTRL_ENABLE_MASK		BIT(0)
#define WDT_CTRL_ENABLE			1
#define WDT_CTRL_DISABLE		0
#define WDT_CTRL_MODE_MASK		BIT(1)
#define WDT_CTRL_MODE_RESET		1
#define WDT_CTRL_EXT_RESET_MASK		BIT(4)
#define WDT_CTRL_EXT_RESET_DISABLE	0
#define WDT_CTRL_RESET_NUM0_MASK	BIT(5)
#define WDT_CTRL_RESET_NUM0_ENABLE	1
#define WDT_CTRL_MSB_MASK		GENMASK(15, 8)
#define WDT_CTRL_KEY_MASK		GENMASK(31, 16)
#define WDT_CTRL_KEY_VALUE		0x5a96

#define WDT_STATUS			0x04
#define WDT_STATUS_COUNT_MASK		GENMASK(19, 0)
#define WDT_STATUS_ENABLED_MASK		BIT(30)

#define WDT_TRIGGER			0x08
#define WDT_TRIGGER_MASK		BIT(0)
#define WDT_TRIGGER_RELOAD		1

#define WDT_MANUAL_RESET		0x0c
#define WDT_MANUAL_RESET_MASK		BIT(0)
#define WDT_MANUAL_RESET_ASSERT		1

#define WDT_DEFAULT_TIMEOUT		80U
#define WDT_PRESCALE			1024
#define WDT_MSB_TICKS			4096
#define WDT_POLL_DELAY_US		1
#define WDT_POLL_TIMEOUT_US		100
...
static int novatek_wdt_ping(struct watchdog_device *wdd)
{
	...
	ret = readl_poll_timeout_atomic(wdt->base + WDT_TRIGGER,
					reg,
					!(reg & WDT_TRIGGER_MASK),
					WDT_POLL_DELAY_US,
					WDT_POLL_TIMEOUT_US);
	if (!ret)
		writel(FIELD_PREP(WDT_TRIGGER_MASK,
				  WDT_TRIGGER_RELOAD),
		       wdt->base + WDT_TRIGGER);
				   ...
				   unsigned int timeout)
{
	...
	msb = div64_u64((u64)timeout * wdt->rate,
			(u64)wdt->soc->prescale * WDT_MSB_TICKS);
	if (msb > FIELD_MAX(WDT_CTRL_MSB_MASK))
		return -EINVAL;

	spin_lock_irqsave(&wdt->lock, flags);
	reg = readl(wdt->base + WDT_CTRL);
	reg &= ~(WDT_CTRL_MSB_MASK | WDT_CTRL_KEY_MASK);
	reg |= FIELD_PREP(WDT_CTRL_MSB_MASK, msb) |
	       FIELD_PREP(WDT_CTRL_KEY_MASK,
			  WDT_CTRL_KEY_VALUE);
...
static int novatek_wdt_start(struct watchdog_device *wdd)
{
	...
	spin_lock_irqsave(&wdt->lock, flags);
	reg = readl(wdt->base + WDT_CTRL);
	reg &= ~(WDT_CTRL_ENABLE_MASK |
		 WDT_CTRL_MODE_MASK |
		 WDT_CTRL_EXT_RESET_MASK |
		 WDT_CTRL_KEY_MASK);
	reg |= FIELD_PREP(WDT_CTRL_ENABLE_MASK,
			  WDT_CTRL_DISABLE) |
	       FIELD_PREP(WDT_CTRL_MODE_MASK,
			  WDT_CTRL_MODE_RESET) |
	       FIELD_PREP(WDT_CTRL_EXT_RESET_MASK,
			  WDT_CTRL_EXT_RESET_DISABLE) |
	       FIELD_PREP(WDT_CTRL_RESET_NUM0_MASK,
			  WDT_CTRL_RESET_NUM0_ENABLE) |
	       FIELD_PREP(WDT_CTRL_KEY_MASK,
			  WDT_CTRL_KEY_VALUE);
	writel(reg, wdt->base + WDT_CTRL);
	ret = readl_poll_timeout_atomic(wdt->base + WDT_STATUS,
					status,
					!(status &
					  WDT_STATUS_ENABLED_MASK),
					WDT_POLL_DELAY_US,
					WDT_POLL_TIMEOUT_US);
	if (!ret) {
		...
		ret = readl_poll_timeout_atomic(
			wdt->base + WDT_STATUS, status,
			status & WDT_STATUS_ENABLED_MASK,
			WDT_POLL_DELAY_US,
			WDT_POLL_TIMEOUT_US);
	}
	spin_unlock_irqrestore(&wdt->lock, flags);
...
static int novatek_wdt_stop(struct watchdog_device *wdd)
{
	...
	spin_lock_irqsave(&wdt->lock, flags);
	reg = readl(wdt->base + WDT_CTRL);
	reg &= ~(WDT_CTRL_ENABLE_MASK | WDT_CTRL_KEY_MASK);
	reg |= FIELD_PREP(WDT_CTRL_ENABLE_MASK,
			  WDT_CTRL_DISABLE) |
	       FIELD_PREP(WDT_CTRL_KEY_MASK,
			  WDT_CTRL_KEY_VALUE);
...
static unsigned int novatek_wdt_get_timeleft(struct watchdog_device *wdd)
{
	...
	count = readl(wdt->base + WDT_STATUS) &
		WDT_STATUS_COUNT_MASK;
			       ...
			       unsigned long action, void *data)
{
	...
	reg = readl(wdt->base + WDT_CTRL);
	reg &= ~(WDT_CTRL_ENABLE_MASK | WDT_CTRL_KEY_MASK);
	reg |= FIELD_PREP(WDT_CTRL_KEY_MASK,
			  WDT_CTRL_KEY_VALUE);
	writel(reg, wdt->base + WDT_CTRL);
	ret = readl_poll_timeout_atomic(wdt->base + WDT_STATUS,
					status,
	...
	reg &= ~(WDT_CTRL_MODE_MASK |
		 WDT_CTRL_EXT_RESET_MASK |
		 WDT_CTRL_MSB_MASK);
	reg |= FIELD_PREP(WDT_CTRL_MODE_MASK,
			  WDT_CTRL_MODE_RESET) |
	       FIELD_PREP(WDT_CTRL_EXT_RESET_MASK,
			  WDT_CTRL_EXT_RESET_DISABLE) |
	       FIELD_PREP(WDT_CTRL_RESET_NUM0_MASK,
			  WDT_CTRL_RESET_NUM0_ENABLE) |
	       FIELD_PREP(WDT_CTRL_MSB_MASK, 1);
	writel(reg, wdt->base + WDT_CTRL);
	udelay(wdt->enable_delay_us);
	...
	writel(FIELD_PREP(WDT_MANUAL_RESET_MASK,
			  WDT_MANUAL_RESET_ASSERT),
	       wdt->base + WDT_MANUAL_RESET);
	readl(wdt->base + WDT_MANUAL_RESET);
	mdelay(100);
	ret = -ETIMEDOUT;
...
static int novatek_wdt_probe(struct platform_device *pdev)
{
	...
	wdt->wdd.max_timeout = div64_u64(
		(u64)(FIELD_MAX(WDT_CTRL_MSB_MASK) + 1) *
		wdt->soc->prescale * WDT_MSB_TICKS,
		wdt->rate);
